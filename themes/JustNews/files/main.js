  ...
  8 : [function(baseTemp, canCreateDiscussions, a) {
    ...
    !function($) {
      ...
      $(document).ready(function() {
        ...
        const observer = new MutationObserver((mutationsList, obs) => {
          for (let mutation of mutationsList) {
              if (mutation.type === 'childList') {
                  if (mutation.target.matches('.widget_shopping_cart_content, .woocommerce-cart-form')) {
                      $(mutation.target).find(".j-lazy").lazyload({
                          webp: webpPngbinary,
                          threshold: 250,
                          effect: "fadeIn"
                      });
                      scrollElem.trigger("scroll");
                  } else if (mutation.target.matches('header.header')) {
                      setup_doc_cloud_size_hacks();
                  }
              } else if (mutation.type === 'subtree') {
                  if (mutation.target.matches('header.header .wpcom-adv-menu>li>a>img')) {
                      setTimeout(() => {
                          setup_doc_cloud_size_hacks();
                      }, 300);
                  }
        ...
        // 选择需要观察的 DOM 节点
        const targetNodes = document.querySelectorAll('.widget_shopping_cart_content, .woocommerce-cart-form, header.header, .wpcom-adv-menu>li>a>img');
  ...
  20 : [function(saveNotifs, canCreateDiscussions, a) {
    ...
    (function($) {
      ...
      $(document).ready(function() {
        ...
        const observer = new MutationObserver((mutationsList, observer) => {
          for (let mutation of mutationsList) {
              if (mutation.type === 'childList') {
                  ...
                  if (mutation.target.matches('.navbar-action')) {
                    parseFonts();
                  }
              } else if (mutation.type === 'subtree') {
                  for (let addedNode of mutation.addedNodes) {
        ...
        // 选择需要观察的节点
        const targetNodes = document.querySelectorAll('body, .navbar-action');
        targetNodes.forEach(node => observer.observe(node, config));
        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

        if (
          $("body").on("click", ".kx-new", function() {
            /** @type {string} */
          ...
          _wpcom_js.fixed_sidebar && "1" == _wpcom_js.fixed_sidebar && s.length && s.find(".widget").length && $this.width() > 991
        ) {
          /** @type {number} */
          var i = 0;
          for (; i < s.length; i++) {
