...
const OPT = {  // 网站配置
    ...
    "siteName": "Tinger Blog",  // 博客名称
    "siteDescription": "Tinger Blog on Cloudflare Worker",  // 博客描述
    "keyWords": "cloudflare,KV,workers,blog",  // 关键字
    "logo": `${Prefix}/themes/JustNews/files/logo2.png`,  // JustNews主题的logo
    ...
    "cacheTime": 60 * 60 * 24,  // 文章在浏览器的缓存时长(秒),建议=文章更新频率
    "html404": `<b>404</b>`,  // 404页面代码
    "codeBeforHead": `<script src="${Prefix}/static/jquery-3.5.1.min.js"></script>`,  // 其他代码,显示在</head>前
...
<a href="https://github.com/Tinger-X/blog">Tinger Blog</a> &
<a href="https://github.com/Arronlong/cfblog-plus">CF-Blog-Plus</a> &
<a href="https://github.com/gdtool/cloudflare-workers-blog">CF-Workers-Blog</a>`,  // 自定义版权信息,建议保留大公无私的 Coudflare 和 作者 的链接
    "robots": `User-agent: *
...
// ---对部分配置进行处理---
{
    ...
    if (!OPT.themeURL.endsWith("/")) {
        OPT.themeURL = OPT.themeURL + "/";
    }
    if (!OPT.theme_github_path.endsWith("/")) {
...
async function handlerRequest(event) {
    ...
    // 组装请求url，查看是否有缓存
    const D = caches.default, M = "https://" + OPT.siteDomain + url.pathname, x = new Request(M, request);
...
async function handle_search(request) {
    // 如果设置了参数，则使用参数指定的url，可使用github action方式自动定期更新
    let xml;
...
async function renderBlog(url) {
    ...
    // 处理主题预览及分页
    let theme = url.searchParams.get("theme"), pageSize = url.searchParams.get("pageSize");
    if (theme) {
    ...
    cfg.widgetMenuList = menus,// 导航
        ...
        cfg.prefix = Prefix,
        cfg.OPT = OPT;  // 使用mustache.js进行页面渲染（参数替换）
...
async function handle_article(id) {
    ...
    cfg.widgetMenuList = menus,// 导航
        ...
        cfg.keyWords = keyWord,// SEO关键字
        cfg.prefix = Prefix,
        cfg.OPT = OPT;  // 使用mustache.js渲染页面（参数替换）
...
async function parseReq(request) {
    ...
    if (content_type.includes("form")) {
        const formData = await request.formData(), ret = {};
...
// @ts-ignore 引入mustache.js，4.1.0：https://cdn.bootcdn.net/ajax/libs/mustache.js/4.1.0/mustache.min.js
